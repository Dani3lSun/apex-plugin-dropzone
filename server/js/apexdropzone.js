...
var apexDropzone = {
  ...
  imageExists: function(url, good, bad) {
    try {
      ...
      apex.debug.log('imageExists Error', err);
    }
  },
  ...
  // resize images
  resizeImage: function(pDropzone, pFile, callback) {
  ...
  uploadDzFiles: function(pRegionId, pDropzone, pFiles, pAjaxIdentifier, pWaitTime) {
    ...
    for (var i = 0; i < pFiles.length; i++) {
      ...
      apexDropzone.resizeImage(pDropzone, pFiles[i], function(resizedFile) {
        // resize image: save original file object / resizedFile for fileReader
        if (resizedFile) {
          var orgFile = file;
          file = resizedFile;
        }
              ...
              file.xhr = apex.jQuery.ajax({
                ...
                xhr: function() {
                  ...
                  if (XhrObj.upload) {
                    ...
                    apex.debug.log('uploadDzFiles XHR', 'Upload progress is not supported.');
                  }
                  return XhrObj;
                }
              });
  ...
  uploadDzFilesChunked: function(pRegionId, pDropzone, pFiles, pAjaxIdentifier, pWaitTime, pChunkSize) {
    // go through files
    for (var i = 0; i < pFiles.length; i++) {
      var file = pFiles[i];
      var orgFile;
                    ...
                    file.xhr = apex.jQuery.ajax({
                      ...
                      xhr: function() {
                        ...
                        if (XhrObj.upload) {
                          ...
                          apex.debug.log('uploadDzFilesChunked XHR', 'Upload progress is not supported.');
                        }
                        return XhrObj;
                      }
                    });
  ...
  apexDropzoneFnc: function(pRegionId, pOptions) {
    ...
    var vResizeImages = apexDropzone.parseBoolean(vOptions.resizeImages);
    var vResizeWidth = parseInt(vOptions.resizeWidth);
    var vResizeHeight = parseInt(vOptions.resizeHeight);
    var vPluginPrefix = vOptions.pluginPrefix;
    var vChunkSize = parseInt(vOptions.chunkSize);
    ...
    apex.debug.log('apexDropzoneFnc - vDeleteFiles', vDeleteFiles);
    apex.debug.log('apexDropzoneFnc - vResizeImages', vResizeImages);
    apex.debug.log('apexDropzoneFnc - vResizeWidth', vResizeWidth);
    apex.debug.log('apexDropzoneFnc - vResizeHeight', vResizeHeight);
    apex.debug.log('apexDropzoneFnc - vPluginPrefix', vPluginPrefix);
    apex.debug.log('apexDropzoneFnc - vChunkSize', vChunkSize);
    apex.debug.log('apexDropzoneFnc - vDisplayMessage', vDisplayMessage);
    apex.debug.log('apexDropzoneFnc - vFallbackMessage', vFallbackMessage);
    ...
    var lResizeHeight;
    if (vResizeImages) {
      lResizeWidth = vResizeWidth || null;
      lResizeHeight = vResizeHeight || null;
    } else {
      lResizeWidth = null;
      lResizeHeight = null;
    }
    ...
    var dzOptions = {
      ...
      dictFallbackMessage: vFallbackMessage,
    ...
    var myDropzone = new Dropzone('div#' + vRegion, dzOptions);
    ...
    myDropzone.uploadFiles = function(files) {
      if (vUploadMechanism == 'NORMAL') {
        ...
        apexDropzone.uploadDzFilesChunked(pRegionId, myDropzone, files, vAjaxIdentifier, vWaitTime, vChunkSize);
      }
